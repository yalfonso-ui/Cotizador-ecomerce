...
test.describe('StepDestination — selección múltiple de destinos', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await expect(page.locator('[data-destination-root]')).toBeVisible()
  })

  test('1. El dropdown se abre al hacer clic en el input principal', async ({ page }) => {
    const root = page.locator('[data-destination-root]')

    await expect(root.locator('input[placeholder="¿A dónde viajas?"]')).toBeHidden()
    await expect(root.getByRole('button', { name: 'Hecho' })).toHaveCount(0)
  ...
  test('3. Seleccionar múltiples países muestra etiquetas; "Hecho" cierra y actualiza el input principal', async ({ page }) => {
    ...
    await expect(root.getByText(/Estados Unidos, Francia \+1/)).toBeVisible()

    await expect(root.getByRole('button', { name: 'Continuar' })).toBeEnabled()
  })
  ...
  test('4. El botón limpiar (x) del input principal vacía toda la selección', async ({ page }) => {
    ...
    await expect(root.locator('input[placeholder="¿A dónde viajas?"]')).toBeVisible()
    await expect(root.getByRole('button', { name: 'Continuar' })).toBeDisabled()
  })
