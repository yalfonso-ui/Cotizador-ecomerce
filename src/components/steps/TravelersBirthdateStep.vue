...
<template>
  <div class="space-y-5">
    <div
      ...
      class="bg-gradient-to-r from-cyan-50 to-blue-50 rounded-xl p-4 border border-cyan-100"
      role="note"
    >
    ...
    <div
      ...
      :key="index"
      class="bg-white border-2 border-slate-100 rounded-2xl p-4 space-y-3"
    >
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-2.5">
          <div class="w-8 h-8 rounded-full bg-cyan-50 flex items-center justify-center shrink-0">
            <span class="text-sm font-bold text-cyan-600">{{ index + 1 }}</span>
          </div>
          <div>
        ...
        <div v-if="ages[index] !== null" class="text-right">
          ...
          <p class="text-lg font-black text-cyan-600">{{ ages[index] }} años</p>
        </div>
      ...
      <div>
        ...
        <div class="relative">
          <input
            ...
            class="w-full h-11 px-3.5 bg-slate-50 border-2 border-slate-200 rounded-lg text-slate-700 placeholder:text-slate-300 transition-all focus:bg-white focus:border-cyan-400 focus:ring-4 focus:ring-cyan-50 outline-none text-sm font-semibold tracking-wider"
          />
        </div>
      </div>
      ...
      <button type="button"
        ...
        class="inline-flex items-center gap-1.5 text-xs font-medium text-slate-400 hover:text-red-500 transition-colors"
      >
    ...
    <button type="button"
      ...
      @click="addTraveler"
      class="w-full inline-flex items-center justify-center gap-2 py-3 bg-cyan-50 hover:bg-cyan-100 border-2 border-dashed border-cyan-300 text-cyan-700 font-semibold text-sm rounded-2xl transition-all"
    >
    ...
    >
      ...
      <svg v-if="allValid" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
